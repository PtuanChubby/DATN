...
(function (factory) {
  ...
  const defaults = {
    ...
    style: {
      ...
      navCss: "nav",
      navLinkCss: "nav-link",
      contentCss: "form-content",
      contentPanelCss: "form-pane",
  ...
  class SmartWizard {
    ...
    _getShowable(idx, dir) {
      ...
      const elmList =
        dir == "prev"
          ? $(this.steps.slice(0, idx).get().reverse())
          : this.steps.slice(idx + 1);
  ...
  $.fn.smartWizard.transitions = {
    ...
    slideHorizontal: (
      ...
      const animFn = (elm, iniLeft, finLeft, cb) => {
        elm
          .css({ position: "absolute", left: iniLeft })
          .show()
          .animate(
            { left: finLeft },
            wizardObj.options.transition.speed,
    ...
    slideVertical: (
      ...
      const animFn = (elm, iniTop, finTop, cb) => {
        elm
          .css({ position: "absolute", top: iniTop })
          .show()
          .animate(
            { top: finTop },
            wizardObj.options.transition.speed,
